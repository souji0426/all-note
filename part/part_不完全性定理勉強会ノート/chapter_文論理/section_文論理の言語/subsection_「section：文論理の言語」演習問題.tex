...
次のExerciseを証明するためのPropositionを証明しておく.
...
\begin{proof}
 ...
 $\alpha = \mathcal{E}_{\land}(\beta_1, \beta_2)$となっていたとすると,
...
\begin{proof}
 ...
 \begin{itemize}
   ...
   \item どんな長さが6の表現も整式でないこと \\
     $\alpha$を長さが6である表現とする.
     よって構成列$\ntuple{\varepsilon_1}{\varepsilon_n}$が存在して$\alpha=\varepsilon_n$となっている.
     ...
     \end{itemize}
     $\varepsilon_n$の長さが$6$であることから$\varepsilon_n\notin \SetOfSymbol$,
     つまり($1$)ではない. \\
     そして($2$)でもない.
     \begin{sub-block}{{\boldmath $\because$}\,}
       もし($2$)だったとしてそんな$\varepsilon_j$を固定する.
       \PropRef{proposition:構成列の始切片は構成列}より$\ntuple{\varepsilon_1}{\varepsilon_j}$も構成列であり,
       ゆえに$\varepsilon_j$は以下のいずれかを満たす
       \begin{itemize}
         \item[($2-1$)] $\varepsilon_j\in \SetOfSymbol$.
         \item[($2-2$)] $\exists j^{\prime}<n(\ \varepsilon_j = \mathcal{E}_{\lnot}( \varepsilon_{j^{\prime}} )\ )$.
         \item[($2-3$)] $\exists j^{\prime},k^{\prime}<n(\ \varepsilon_j = \mathcal{E}_{\square}( \varepsilon_{j^{\prime}},\varepsilon_{k^{\prime}} )\ )$.
       \end{itemize}
       $\varepsilon_j$は$\varepsilon_n$よりも$(,),\lnot$の3つ分長さが短い,
       つまり長さが$3$なので,
       先ほどと同じ理由で($2-1$)ということはない. \\
       もし($2-2$)だったとしてそんな$\varepsilon_{j^{\prime}}$が存在したとすると,
       先ほどと同じ理由で$\varepsilon_{j^{\prime}}$は$\varepsilon_j$よりもさらに長さが$3$短くなるが,
       そうなると$\varepsilon_{j^{\prime}}$は長さが$0$になり,
       整式とはならず矛盾. \\
       もし($2-3$)だったとしてそんな$\varepsilon_{j^{\prime}},\varepsilon_{k^{\prime}}$が存在したとすると,
       上と同じ理由でどちらも長さが$0$となった同じ理由で矛盾.
     \end{sub-block}

     そして($3$)でもない.
     \begin{sub-block}{{\boldmath $\because$}\,}
       もし($3$)だったとして,
       そんな$\varepsilon_j,\varepsilon_k$が存在したとする.
       するとどちらかの長さが$2$になるが,
       \PropRef{proposition:式構成操作を行った整式は長さが4以上}よりそんな整式は存在しないため矛盾.
     \end{sub-block}
...
上の証明で長さが$2,3,6$の整式が存在しないことは帰納法の原理を用いて示すこともできそうなので
やってみることにします.

\begin{proof}
  $\mathcal{A}\subseteq \SetOfWff$を
  \begin{equation*}
    \mathcal{A}\ =\ \SetBar{\alpha}{\alpha\text{の長さは}2,3,6\text{でない}}
  \end{equation*}
  と定め,
  $\mathcal{A}=\SetOfWff$であることを帰納法の原理を用いて示す. \\
  $\SetOfSentenceSymbol \subseteq \mathcal{A}$は明らかなので,
  $\mathcal{A}$が式構成操作で閉じていることを示す.
  任意に$\alpha,\beta\in \mathcal{A}$をとる.
  $\gamma = \mathcal{E}_{\lnot}( \alpha )$とおくと,
  $\gamma$の長さは$\alpha$よりも$3$長い.
  すると$\gamma$の長さは$4$以上,
  つまり$2,3$ではなく,
  そして$\alpha$の長さが$3$でなくことから,
  $\gamma$の長さは$6$でもない,
  よって$\gamma\in \mathcal{A}$,
  つまり$\mathcal{E}_{\lnot}( \alpha ) \in \mathcal{A}$である.
  続いて$\gamma = \mathcal{E}_{\lnot}( \alpha,\beta )$とおくと,
  先ほど同じ理由で$\gamma$の長さは$2,3$ではない.
  そして$\gamma$の長さが$6$ならば$\alpha,\beta$のどちらかの長さが$2$となるが,
  これはどちらも$\mathcal{A}$に属することに矛盾する.
  よって$\gamma$の長さは$2,3,6$のいずれでもなく,
  ゆえに$\mathcal{E}_{\lnot}( \alpha,\beta )\in \mathcal{A}$である.
  残りの式構成操作についても同様に示せるので省略する.
\end{proof}
...
\begin{proof}
 ...
 以下の2つのことを示せば帰納法の原理である\ThRef{theorem:文論理における帰納法の原理}より証明完了となる.
 \begin{itemize}
...
\begin{proof}
 ...
 $\varphi$は$A_4$を含んでいないので上記の取り去る作業のあとも構成列に残っており,
 列ので位置も末項のまま,
 つまり$\varepsilon^{\prime}_m=\varphi$である.
...
私は勉強会にて上記の証明を発表しましたが,
参加者の方からのコメントで帰納法の原理でも示すやり方を提案されたので,
そちらも証明を書いてみます.

\begin{proof}
  $\alpha\in \SetOfWff$と$k\in \mathbb{N}$に対して,
  2つの主張$(\text{a})_{\alpha,k},(\text{b})_{\alpha,k}$を以下のように定める.
  \begin{itemize}
    \item[] $(\text{a})_{\alpha,k}$は「$\alpha$の長さは$4k+1$である」の略記とする.
    \item[] $(\text{b})_{\alpha,k}$は「$\alpha$を構成する記号のうち,
      文記号の個数は$k+1$個である」の略記とする.
  \end{itemize}
  そして$\mathcal{A}\subseteq \SetOfWff$を
  \begin{equation*}
    \mathcal{A}\ =\ \SetBar{\alpha}{\alpha:\lnot\text{を含まない}\ \rightarrow\ \exists k\in \mathbb{N}\big( \ (\text{a})_{\alpha,k}\land (\text{b})_{\alpha,k}\ \big)}
  \end{equation*}
  と定める.
  $\mathcal{A}=\SetOfWff$であることを示せば,
  先の証明と同じ理由から証明完了となる.
  そして$\mathcal{A}=\SetOfWff$であることを帰納法の原理を用いて示す.
  \begin{itemize}
    \item $\mathcal{A}$にはすべての文記号が属すること \\
      任意に文記号$A$をとる.
      整式$\alpha$を$A$1文字の整式とする.
      $\alpha$は明らかに$\lnot$を含んでいない.
      すると$k=0$とすれば$(\text{a})_{\alpha,k},(\text{b})_{\alpha,k}$のどちらも真であり,
      そんな$k$の存在から$\alpha\in \mathcal{A}$,
      つまり$A\in \mathcal{A}$.
    \item $\mathcal{A}$がすべての式構成操作について閉じていること \\
      任意に$\alpha,\beta\in \mathcal{A}$をとる.
      $\gamma=\mathcal{E}_{\lnot}(\alpha)$とおくと,
      $\gamma$は$\lnot$記号を含むため
      \begin{equation*}
        \gamma:\lnot\text{を含まない}\ \rightarrow\ \exists k\in \mathbb{N}\big( \ (\text{a})_{\gamma,k}\land (\text{b})_{\gamma,k}\ \big)
      \end{equation*}
      は真であり,
      よって$\gamma\in \mathcal{A}$,
      つまり$\mathcal{E}_{\lnot}(\alpha)\in \mathcal{A}$. \\
      続いて$\gamma=\mathcal{E}_{\land}(\alpha, \beta)$とおく.
      もし$\alpha,\beta$のどちらかに$\lnot$が含まれていれば,
      $\gamma$にも含まれているので上と同じ理由で$\mathcal{E}_{\land}(\alpha, \beta)\in \mathcal{A}$.
      $\alpha,\beta$のどちらもに$\lnot$が含まれていないとすると$\gamma$にも含まれていない.
      そして$\alpha\in \mathcal{A}$より$\exists k\in \mathbb{N}\big( \ (\text{a})_{\alpha,k}\land (\text{b})_{\alpha,k}\ \big)$が成り立っているので,
      そんな$k$を改めて$k_{\alpha}$として固定する.
      同様に$k_{\beta}$も固定する.
      そして$k_{\gamma}=k_{\alpha}+k_{\beta}+1$とおく.
      すると$\gamma$の長さは$\alpha,\beta$の長さの合計に加えて$(,\land,)$の3つ分長くなっている,
      つまり
      \begin{align*}
        (4 k_{\alpha}+1)\ +\ (4 k_{\beta}+1)\ +\ 3\  &=\  4( k_{\alpha}+k_{\beta}+1)\ +\ 1 \\
        &=\ 4 k_{\gamma}+1
      \end{align*}
      つまり$(\text{a})_{\gamma,k_{\gamma}}$が成立.
      そして$\gamma$を構成する記号のうち文記号の個数は$\alpha,\beta$のそれの合計なので,
      \begin{equation*}
        ( k_{\alpha}+1 )\ +\ ( k_{\beta}+1)\ =\ k_{\gamma}+1
      \end{equation*}
      よって$(\text{b})_{\gamma,k_{\gamma}}$が成立.
      上記の2つの主張が成り立つような$k_{\gamma}$の存在から$\gamma\in \mathcal{A}$,
      つまり$\mathcal{E}_{\land}(\alpha, \beta)\in \mathcal{A}$. \\
      他の式構成操作に関しても同様に示せるので省略する. \qedhere
  \end{itemize}
\end{proof}
