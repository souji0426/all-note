...
\begin{proposition}\label{prop:式構成操作を行った整式は長さが4以上}
...
\begin{proposition}\label{prop:整式でない表現の式構成操作の結果も整式でない}
...
\begin{proof}
 ...
 \begin{itemize}
   \item 長さが2または3のどんな表現も整式でないこと \\
     ...
     何らかの式構成操作の結果であるが,
     \PropRef{prop:式構成操作を行った整式は長さが4以上}よりいずれの式構成操作の結果も長さが4以上になり,
   ...
   \item どんな長さが6の表現も整式でないこと \\
     ...
     しかし長さが3の整式は存在しないため,
     $\beta$も整式でない.
     そして\PropRef{prop:整式でない表現の式構成操作の結果も整式でない}より$\alpha$も整式とならない. \\
...
\begin{proof}
 ...
 $\mathcal{A}\subseteq \mathcal{F}$を否定記号$\lnot$を含まない整式の集合とする.
 整式に含まれる左括弧記号の個数
 \footnote{
   別の言い方をすれば,
