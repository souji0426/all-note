...
よって私が好きな本である\cite{Logic005:todayama}から引用する.
...
\begin{definition}[記号(E:p13~14,\ K:p20)]\label{def:記号}
...
\begin{enumerate}
 \item
   ...
   また一階述語述語論理における区切り文字については後で忘れないように書くこと
   \index{あとでかく@後で書く}
 \item
   ...
   \footnote{
     とはいえ昔とある銀行ATMに同じように案内されて通帳とカード両方を入れてみたら「最初からやりなおしてください」と言われたことがある.
 ...
 \item \textgt{いくつかの記号の総称について} \\
   ...
   \footnote{
     テキストでは定義していない言葉ではあるけれど,
     なんとなく意味はわかるし不要かとも思ったけどもあえて定義しておいた.
     また$\lnot$は結合記号とよぶべきでしょうが,
     そんな記号は1つしかないため総称は必要ないと判断しました.
   }
   ...
   \footnote{
     ...
     ただここでのpropositionは証明すべき主張にしか使わず,
     ゆえにノートでは他の定義や定理たちと同じように下線を引いていたり太字になっていたりするので紛らわしさはないので,
     併用しても問題はないのですが,
 ...
 \item \textgt{「互いに区別できる」部分について} \\
   ...
   \footnote{
     とは書いてみたもののあまり自信がない.
     なぜなら人によって区別できない記号って「字が汚い」場合以外どんなことがあるのだろうか.
 ...
 \item \label{remark:symbolの定義のオブジェクトについて}
   ...
   \footnote{
     ...
     現実のものに限れば無限個用意できないため不適ではなかろうか.
     仮に無限個用意できても別の注意にある「互いに区別できる」部分が人にとっては難しそうだ.
     まず人間が見分けできる色が高々有限色しかないだろうし.
     そう判断したのは日本語での色の名前が有限しかないことから（\url{https://ja.wikipedia.org/wiki/\%E6\%97\%A5\%E6\%9C\%AC\%E3\%81\%AE\%E8\%89\%B2\%E3\%81\%AE\%E4\%B8\%80\%E8\%A6\%A7}）.
     ...
     これもやってもよいとはいってもやる人はまずいないと思うかなりおまのじゃくな例え話.
   }
   ...
   \footnote{
     ...
     だからここらへんの例え話もコンピュータに関する教養が足りず少し自信がない.
   }
 ...
 \item \textgt{「無限個のオブジェクトの列」部分と「列の第一成分から」部分について}
   ...
   \footnote{
     ...
     ちなみにハッシュ関数という言葉はWikipedia（\url{https://ja.wikipedia.org/wiki/\%E3\%83\%8F\%E3\%83\%83\%E3\%82\%B7\%E3\%83\%A5\%E9\%96\%A2\%E6\%95\%B0\#\%E8\%AA\%9E\%E6\%BA\%90}）より拝借した（自分は普段は単にハッシュと呼んでるけれども）.
   }
   ...
   だからこそDefinition \ref{def:記号}においても「列の第一成分から」とテキストにはないものを追加した.
 ...
 \item \label{remark:symbolの定義の「これらの記号のどれもが他の記号の有限な長さの列とは一致しない」部分}
   ...
   もし2つの記号の列が$\ntuple{a_1}{a_m}=\ntuple{b_1}{b_n}$でDefinition \ref{def:記号}の条件をみたす記号ならば,
   $m=n$かつ$a_i=b_i$となる.
 \item
...
\begin{definition}[表現(E:p15,\ K:p23)]\label{def:表現}
 ...
 \begin{itemize}
   ...
   \item[3.]
     ...
     \footnote{
       これはテキストでは定義されていない言葉ではあるが,
       定義しておくと便利かと思ったのでプログラミングにおける文字列結合演算を参考に定義した
...
Definition \ref{def:表現}でも使ったが改めて注意すると,
...
\begin{definition}[（素朴な）整式の定義(E:p16~17,\ K:p25)]\label{def:（素朴な）整式の定義}
 ...
 \footnote{
   テキストでは単に「式（formula）」とか原著だと「wff」などの呼び方も提示されているが,
   個人的な理由で整式のみで統一する.
 }
...
しかしより厳密に定義することもできる.
そうしたとき式構成操作を$\mathcal{E}$上の関数として捉えることにも意味がでてくる. \\
簡単にいうと整式とは文記号から始めて式構成操作を有限回適用して構成できる表現のことと言えるが,
この「有限回適用して」の部分を構成列というものを使って厳密に定義できる. \\
1.4節ではこれについてもう一度触れることがあるようなのであとで書くことにする\index{あとでかく@後で書く}.

\begin{definition}[構成列と整式(E:p17~18,\ K:p26~27)]\label{def:構成列と整式}
 ...
 \footnote{
   これもテキストではこの記号を与えたりはしていないが,
   もしかしたら便利なこともあるかもしれないので定めておいた.
   本来のテキストにある記号と衝突しないかぎりは今後も使っていく.
 }
\end{definition}

いまだけの記法として$\mathcal{P}$を文記号の集合,
$\mathcal{E}_2$を2項結合記号の集合とするとき,
有限列$\ntuple{\varepsilon_1}{\varepsilon_n}$が構成列であることは論理式で
\begin{equation*}
 \forall i<n\big(\ \varepsilon_i\in \mathcal{P}\ \lor\ \exists j< i( \varepsilon_i=\mathcal{E}_{\lnot}( \varepsilon_j ) )\ \lor\ \exists j,k< i\exists \square \in \mathcal{E}_2 ( \varepsilon_i=\mathcal{E}_{\square}( \varepsilon_j, \varepsilon_k )\ \big)
\end{equation*}
と書ける.
...
これが頭の中で完全に区別できている人たちの中で議論のさいにこう書くなら問題はないが（それでも多分良い顔はしないと思うけれど）,
初学者には余計な誤解やそれによる遠回りを与える可能性がある.
ゆえにテキストでは（おそらく意図的に）一貫して（\ChapRef{chapter:導入}のような別に論理学に直接関係する概念でなくても）何かを定義するさいには,
日常言語で述べることにしたのであろう.

\begin{example}\label{example:構成列}
...
\ChapRef{chapter:導入}によると木概念は非形式的な議論にしか使わないと書いてある.
つまり系統樹を使って説明されている事柄はすべて定義に戻ればより厳密な数学的議論をすることができるということなので,
このノートでは系統樹を使った説明はしない.
\footnote{
...
ここからさらにテキストに（この時点で）ない注意を例え話を入れながら追加する.
\begin{enumerate}
 \item
   ...
   たとえば$S_1=\langle A_1,A_2,A_3,(A_1\land A_2)\rangle$や$S_2=\langle A_1,A_2,A_3,(\lnot A_3),(A_1\land A_2)\rangle$も$(A_1\land A_2)$の構成列である.
 ...
 \item
   ...
   例えば$S^{\prime}_1=\singleseq{A_1},\ S^{\prime}_2=\op{A_1}{A_2},\ S^{\prime}_3=\langle A_1,A_2,A_3\rangle,\ S^{\prime}_4=\langle A_1,A_2,A_3,(\lnot A_3)\rangle$は,
   それぞれ$S_2$の始切片で,
   $S^{\prime}_1$は$A_1$の,
   $S^{\prime}_2$は$A_2$の,
   $S^{\prime}_3$は$A_3$の,
   ...
   これは\PropRef{prop:構成列の始切片は構成列}で示した.
 \item
 ...
 \item
   ...
   これはもっと一般的に示せる\index{あとでかく@後で書く}.
 ...
 \item
   \DefRef{def:（素朴な）整式の定義}にある定義を採用するテキストも多い.
   ...
   \footnote{
     この呼び方は\url{https://ja.wikipedia.org/wiki/\%E6\%A7\%8B\%E9\%80\%A0\%E7\%9A\%84\%E5\%B8\%B0\%E7\%B4\%8D\%E6\%B3\%95}より知った.
   }
   これは次の2つのことを示るやり方である.
   \begin{itemize}
   ...
   \end{itemize}
   証明すべきことの名前（Basisとかのこと）は\cite{Logic005:todayama}から拝借した.
   もちろん\cite{Logic005:todayama}では整式の定義は\DefRef{def:（素朴な）整式の定義}と同じようになされている. \\
...
\begin{proposition}\label{prop:構成列の始切片は構成列}
...
\begin{proof}
 ...
 \begin{itemize}
   \item[($1^{\prime}$)] $\varepsilon_m \in \mathcal{P}$.
 ...
 任意の$i\leq m-1$に対して
 \begin{itemize}
   \item[($1^{\prime\prime}$)] $\varepsilon_i \in \mathcal{P}$.
 ...
 任意の$i\leq m$に対して
 \begin{itemize}
   \item[($1$)] $\varepsilon_m \in \mathcal{P}$.
...
\begin{theorem}{帰納法の原理(E:p18 INDUCTION PRINCIPLE,\ K:p27 帰納法の原理)}\label{thm:文論理における帰納法の原理}
 すべての文記号が属し,
...
\begin{enumerate}
 \item
   ...
   集合として$\mathcal{A} = \mathcal{F}$を示すことが証明の目的となる.
   $\mathcal{A}\subseteq \mathcal{F}$であることは明らかなので$\mathcal{F}\subseteq \mathcal{A}$を示すだけでよい.
   部分集合の定義に戻ると,
 ...
 \item
   ...
   論理式で書けば$A\in \mathcal{P}( A \in \mathcal{A})$となる. \\
   また全ての式構成操作について閉じるということをもう少し厳密に見ると,
   $\mathcal{E}_{\lnot}$は$\mathcal{E}$上の1項演算,
   それ以外の式構成操作は$\mathcal{E}$上の2項演算であり,
   $\mathcal{A}\subseteq \mathcal{E}$である.
   たとえば$\mathcal{E}_{\lnot}\colon \mathcal{E}\to \mathcal{E}$について,
...
\begin{proof}
 ...
 \begin{itemize}
   \item[($1$)] $\varepsilon_n \in \mathcal{P}$.
 ...
 \begin{itemize}
   \item[($1$)] $\varepsilon_n \in \mathcal{P}$のとき. \\
     整式$\varepsilon_n$は文記号一文字の整式であり,
     仮定より$\mathcal{P} \subseteq \mathcal{A}$だから$\varepsilon_n\in \mathcal{A}$である.
   \item[($2$)] $\exists j<n\big(\ \varepsilon_n=\mathcal{E}_{\lnot}( \varepsilon_j )\ \big)$のとき. \\
     ...
     \PropRef{prop:構成列の始切片は構成列}より構成列である.
...
\begin{proof}[Theorem \ref{thm:文論理における帰納法の原理}の構造的帰納法による証明]
 Theorem \ref{thm:文論理における帰納法の原理}の下に書いた注意事項を参考にして,
...
\begin{example}[E:p18 EXAMPLE,\ K:p28 例]\label{example:文論理の整式の両括弧の数は同じ}
 どの整式もそれに含まれる右括弧・左括弧の個数は同じである.
 \footnote{
   テキストでは「左括弧の数が右括弧の数より多い表現は整式ではない」だが,
   実際に証明している,
   テキストにて証明末尾に書いてある主張は,
   同じ意味ではあるが,
   ここに書いたものになっていたので,
...
\begin{proof}
 ...
 \end{itemize}
 \ThRef{thm:文論理における帰納法の原理}より$\mathcal{A}=\mathcal{F}$であることがわかる.
 \begin{itemize}
   \item[1.] （$\mathcal{A}$には全ての文記号（1つのみの整式）を含むこと） \\
     任意に$A\in \mathcal{P}$をとる.
     $A$はそれぞれの括弧記号の数は0個,
