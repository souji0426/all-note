...
\begin{proof}
  ...
  $g$はこの$g^{\prime}$を用いて,
  $g(s)=\min \SetBar{ g^{\prime}(s^{\prime}) }{ s^{\prime} \land s^{\prime}=s }$や,
  $g(s) = \min g^{\prime}[ \, \SetBar{s^{\prime}}{s^{\prime} \land s^{\prime}=s} \, ]$と書くことができる. \\
