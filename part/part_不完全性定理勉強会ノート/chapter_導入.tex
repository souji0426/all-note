...
\end{definition}
%s-e::$A;t$::集合に要素を1つ付け足す演算::definition:セミコロン演算
...
\begin{notation}[定義するための記号]
  ...
  \end{itemize}
\end{notation}
%d-e::$\defeq$::オブジェクトを定義するための記号::notetion:定義するための記号
%d-e::$\defarr$::述語を定義するための記号::notetion:定義するための記号
...
%s-u::$\subset$::部分集合関係::definition:部分集合関係
...
%p-o::$\mathcal{P}(X)$::（集合$X$の）べき集合::definition:べき集合
...
%o-r::$\op{x}{y}$::（$x$と$y$の）順序対::definition:順序対
...
%c-a::$X\times Y$::（$X$と$Y$の）直積集合::definition:直積集合
...
%d-o::$\dom(R)$::（関係$R$の）定義域::definition:関係
%r-a::$\ran(R)$::（関係$R$の）値域::definition:関係
%f-i::$\fld(R)$::（関係$R$の）領域::definition:関係
...
%m-a::$\map{f}{X}{Y}$::（集合$X$から$Y$への）写像・関数$f$::definition:写像
%i-n::$\InjectionMap{f}{X}{Y}$::（集合$X$から$Y$への）単射写像$f$::definition:単射
%s-u::$\SurjectionMap{f}{X}{Y}$::（集合$X$から$Y$への）全射写像$f$::definition:全射
%b-i::$\BijectionMap{f}{X}{Y}$::（集合$X$から$Y$への）全単射写像$f$::definition:全単射
%i-d::$\id_X$::（集合$X$上の）恒等写像::definition:恒等写像
...
%e-q::$[x]$::（$x$の）同値類::definition:同値類
%q-u::$A\backslash{R}$::（集合$A$の関係$R$による）商集合::definition:商集合
...
\end{notation}
%n-a::$\mathbb{N}$::自然数全体の集合::notation:自然数全体の集合
