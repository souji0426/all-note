...
\end{definition}
\RegisterInSymbolList{$A;t$}{集合$A$の要素に$t$を加える演算}{definition:セミコロン演算}
...
\begin{notation}[定義するための記号]
  ...
  \end{itemize}
\end{notation}
\RegisterInSymbolList{$\defeq$}{オブジェクトを定義するための記号}{notetion:定義するための記号}
\RegisterInSymbolList{$\defarr$}{述語を定義するための記号}{notetion:定義するための記号}
...
\RegisterInSymbolList{$\emptyset$}{空集合}{definition:空集合}
...
\RegisterInSymbolList{$A\subseteq B$}{集合$A$は$B$の部分集合である}{definition:部分集合関係}
...
\RegisterInSymbolList{$\mathcal{P}(X)$}{集合$X$の）べき集合}{definition:べき集合}
...
\RegisterInSymbolList{$A \cup B$}{集合$A$と$B$の和集合}{definition:和集合}
\index{きょうつうぶぶん@共通部分}\index{intersection}
\RegisterInSymbolList{$A \cap B$}{集合$A$と$B$の共通部分}{definition:共通部分}
\index{まじわらない@（2つの集合が）交わらない}\index{disjoint}
\index{わしゅうごう@（集合族の）和集合}\index{union}
\RegisterInSymbolList{$\bigcup\mathcal{A}$}{集合族$\mathcal{A}$の和集合}{definition:集合族の和集合}
\index{きょうつうぶぶん@（集合族の）共通部分}\index{intersection}
\RegisterInSymbolList{$\bigcap\mathcal{A}$}{集合族$\mathcal{A}$の共通部分}{definition:集合族の共通部分}
...
\RegisterInSymbolList{$\op{x}{y}$}{$x$と$y$の順序対}{definition:順序対}
...
長さの定義に曖昧さがでることはないので,
特に長さの定義を意識する必要はないということである.
最初にこれの注意に該当することは\ThRef{theorem:可算集合の要素の有限列全体も可算}や,
\DefRef{definition:symbol}下の注意事項\ \ref{remark:symbolの定義の「これらの記号のどれもが他の記号の有限な長さの列とは一致しない」部分}があてはまる.
...
\RegisterInSymbolList{$X\times Y$}{集合$X$と$Y$の直積集合}{definition:直積集合}
...
\RegisterInSymbolList{$\dom(R)$}{関係$R$の定義域}{definition:関係}
\RegisterInSymbolList{$\ran(R)$}{関係$R$の値域}{definition:関係}
\RegisterInSymbolList{$\fld(R)$}{関係$R$の領域}{definition:関係}
...
\RegisterInSymbolList{$\map{f}{X}{Y}$}{集合$X$から$Y$への写像・関数}{definition:写像}
\RegisterInSymbolList{$\InjectionMap{f}{X}{Y}$}{集合$X$から$Y$への単射写像$f$}{definition:単射}
\RegisterInSymbolList{$\SurjectionMap{f}{X}{Y}$}{集合$X$から$Y$への全射写像$f$}{definition:全射}
\RegisterInSymbolList{$\BijectionMap{f}{X}{Y}$}{集合$X$から$Y$への全単射写像$f$}{definition:全単射}
\RegisterInSymbolList{$\id_X$}{集合$X$上の恒等写像}{definition:恒等写像}
...
\RegisterInSymbolList{$[x]$}{$x$の同値類}{definition:同値類}
\RegisterInSymbolList{$A\backslash{R}$}{集合$A$の関係$R$による商集合}{definition:商集合}
...
\end{notation}
\RegisterInSymbolList{$\mathbb{N}$}{自然数全体の集合}{notation:自然数全体の集合}
...
\ExRef{example:集合として一致するが長さが異なる有限列}のすぐ下でも書いたが,
有限列は写像でもって定義することもできる.
...
上のように$\min$を使う必要もなくなる.
つまり写像で定義すれば順序対の入れ子構造で定義したときのように,
長さが違うがモノとして異なるような例は,
$A$がどんな集合でも生まれることはない.
なぜなら写像で定義した場合の「列の長さ」とは定義域の要素の数のことであり,
つまり長さが違えば,
それはつまり写像として定義域が異なることになって,
その対応規則がどうあれ一致することはないからである. \\

そうは言い切ったものの,
そのあとで本当にそうなのか疑問に思ってしまった.
なぜなら写像も順序対の集合であり,
そうなると順序対を集合で書き直せば,
集合だけを用いて書き直すことができる（順序対よりも構造がかなり複雑にはなっていると思うが）.
...
\begin{kongo}
  \label{kongo:対応規則は異なるが集合として一致する写像は存在するか？}
...
\subsection*{基数}
\label{subsection:基数}
\subfile{C:/souji/all-note/part/part_不完全性定理勉強会ノート/chapter_集合についての予備知識/subsection_基数.tex}
