...
\end{definition}
%$A;t$::集合$A$の要素に$t$を加える演算::definition:セミコロン演算
...
\begin{notation}[定義するための記号]
  ...
  \end{itemize}
\end{notation}
%$\defeq$::オブジェクトを定義するための記号::notetion:定義するための記号
%$\defarr$::述語を定義するための記号::notetion:定義するための記号
...
%$A\subseteq B$::集合$A$は$B$の部分集合である::definition:部分集合関係
...
%$\mathcal{P}(X)$::（集合$X$の）べき集合::definition:べき集合
...
%$A \cup B$::集合$A$と$B$の和集合::definition:和集合
\index{きょうつうぶぶん@共通部分}\index{intersection}
%$A \cap B$::集合$A$と$B$の共通部分::definition:共通部分
\index{まじわらない@（2つの集合が）交わらない}\index{disjoint}
\index{わしゅうごう@（集合族の）和集合}\index{union}
%$\bigcup\mathcal{A}$::（集合族$\mathcal{A}$の）和集合::definition:集合族の和集合
\index{きょうつうぶぶん@（集合族の）共通部分}\index{intersection}
%$\bigcap\mathcal{A}$::（集合族$\mathcal{A}$の）共通部分::definition:集合族の共通部分
...
%$\op{x}{y}$::（$x$と$y$の）順序対::definition:順序対
...
%$X\times Y$::（集合$X$と$Y$の）直積集合::definition:直積集合
...
%$\dom(R)$::（関係$R$の）定義域::definition:関係
%$\ran(R)$::（関係$R$の）値域::definition:関係
%$\fld(R)$::（関係$R$の）領域::definition:関係
...
%$\map{f}{X}{Y}$::（集合$X$から$Y$への）写像・関数$f$::definition:写像
%$\InjectionMap{f}{X}{Y}$::（集合$X$から$Y$への）単射写像$f$::definition:単射
%$\SurjectionMap{f}{X}{Y}$::（集合$X$から$Y$への）全射写像$f$::definition:全射
%$\BijectionMap{f}{X}{Y}$::（集合$X$から$Y$への）全単射写像$f$::definition:全単射
%$\id_X$::（集合$X$上の）恒等写像::definition:恒等写像
...
%$[x]$::（$x$の）同値類::definition:同値類
%$A\backslash{R}$::（集合$A$の関係$R$による）商集合::definition:商集合
...
\end{notation}
%$\mathbb{N}$::自然数全体の集合::notation:自然数全体の集合
